...
Fortunately, haskell provides an "update syntax" that makes this more reasonable.

```haskell
-- Analogous to emailVar.setDomain("new_domain.com")
let newEmail :: Email
    newEmail = emailVar { domain = "new_domain.com" }
```
...
let newCustomer :: Customer
    newCustomer = customerVar { email = { domain = "new_domain.com" } }
```

We are thus stuck copying fields manually:

```haskell
let -- newEmail = customerVar.getEmail().setDomain("new_domain.com");
    newEmail :: Email
    newEmail = (email customerVar) { domain = "new_domain.com" }

    -- newCustomer = customerVar.setEmail(newEmail)
    newCustomer :: Customer
    newCustomer = customerVar { email = newEmail }
```
...
That is, `emailDomainLens` represents the concept of "getting and setting the field domain" on the type `Email`. This gives us an alternative to the selectors used before:
...
```haskell
let newEmail = (email customerVar) { domain = "new_domain.com" }
    newCustomer = customerVar { email = newEmail }
```

The fundamental problem is that while we have nice syntax for updating `customer.email` and `email.domain`, we cannot compose these to get nice syntax for updating the `customer.email.domain`. Lenses do not have this problem. If we have a `Lens a b` and a `Lens b c`, then we can easily form a `Lens a c`.
